...
def transform(x, y):
    screen_x = int((x + 1) * (w // 2))  # Scale and shift x to screen space
    screen_y = int((1 - y) * (h // 2))  # Scale and shift y to screen space
...
while running:
    ...
    # Plot y = x
    for i in range(w):
        x = (i - w // 2) / (w // 2)  # Scale screen x to interval [-1, 1]
        y = x  # y = x function
